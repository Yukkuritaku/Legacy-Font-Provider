...
public abstract class FolderResourcePackMixin extends AbstractResourcePack implements ResourcePackExt {
    ...
    @Unique
    private void legacyfontprovider$add(File file, int maxDepth, String namespace,
        List<ResourceLocation> resourceLocations, String path, Predicate<String> predicate) {
        File[] afile = file.listFiles();
        if (afile != null) {
            for (File file1 : afile) {
                if (file1.isDirectory()) {
                    if (maxDepth > 0) {
                        this.legacyfontprovider$add(
                            file1,
                            maxDepth - 1,
                            namespace,
                            resourceLocations,
                ...
                } else if (!file1.getName()
                    .endsWith(".mcmeta") && predicate.test(file1.getName())) {
                        try {
    ...
    public Collection<ResourceLocation> legacyfontprovider$getAllResourceLocations(ResourcePackType type, String path,
        ...
        for (String namespace : this.legacyfontprovider$getResourceNamespaces(type)) {
            this.legacyfontprovider$add(
                new File(new File(file, namespace), path),
                maxDepth,
                namespace,
                list,
                path + "/",
